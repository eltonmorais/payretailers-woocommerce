...
include_once( 'class-wc-gateway-payretailers-response.php' );

/**
 * Handles responses from PayRetailers IPN
 */
class WC_Gateway_PayRetailers_IPN_Handler extends WC_Gateway_PayRetailers_Response {
	...
	public function __construct( $sandbox = false, $notify_url = '', $secret_key = '', $invoice_prefix = ''  ) {
		add_action( 'woocommerce_api_wc_gateway_payretailers', array( $this, 'check_response' ) );
		add_action( 'valid-payretailers-standard-ipn-request', array( $this, 'valid_response' ) );
	...
	/**
	 * Check for PayRetailers IPN Response
	 */
	...
	public function valid_response( $posted ) {
		WC_Gateway_PayRetailers::log( 'External order num: ' . $posted['external_order_num'] );
		WC_Gateway_PayRetailers::log( 'Uuid: ' . $posted['uuid'] );
		WC_Gateway_PayRetailers::log( 'Payment status: ' . $posted['status'] );

		$order = $this->get_payretailers_order( $posted, $this->invoice_prefix );
		if ( $order ) {
	...
	/**
	 * Check PayRetailers IPN validity (hmac control)
	 */
	...
	protected function validate_currency( $order, $currency ) {
		...
		if ( $order->get_order_currency() != $currency ) {
			WC_Gateway_PayRetailers::log( 'Payment error: Currencies do not match (sent "' . $order->get_order_currency() . '" | returned "' . $currency . '")' );

			// Put this order on-hold for manual checking
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayRetailers currencies do not match (code %s).', 'payretailers-woocommerce' ), $currency ) );
			exit;
		}
	}
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) != number_format( $amount, 2, '.', '' ) ) {
			WC_Gateway_PayRetailers::log( 'Payment error: Amounts do not match (total: ' . $amount . ') for order #'.$order->id.'('.$order->get_total().')' );

			// Put this order on-hold for manual checking
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayRetailers amounts do not match (total %s).', 'payretailers-woocommerce' ), $amount ) );
			exit;
		}
	}
	...
	protected function payment_status_captured( $order, $posted ) {
		if ( $order->has_status( 'captured' ) ) {
			WC_Gateway_PayRetailers::log( 'Aborting, Order #' . $order->id . ' is already complete.' );
			exit;
		}
		
		$this->validate_currency( $order, $posted['currency'] );
		$this->validate_amount( $order, $posted['grand_total']-$posted['payment_method_fee'] ); 
		$this->save_payretailers_meta_data( $order, $posted );

		if ( 'captured' === $posted['status'] ) {
			$this->payment_complete( $order, ( ! empty( $posted['external_order_num'] ) ? wc_clean( $posted['external_order_num'] ) : '' ), __( 'IPN payment captured', 'payretailers-woocommerce' ) );

			if ( ! empty( $posted['payment_method_fee'] ) ) {
				// log payretailers transaction fee
	...
	protected function payment_status_cancelled( $order, $posted ) {
		$order->update_status( 'cancelled', sprintf( __( 'Payment %s via IPN.', 'payretailers-woocommerce' ), wc_clean( $posted['status'] ) ) );
	}
	...
	protected function payment_status_authorized( $order, $posted ) {
		update_post_meta( $order->id, sprintf( __( 'Payment %s via IPN.', 'payretailers-woocommerce' ), wc_clean( $posted['status'] ) ) );
	}
	...
	protected function payment_status_refunded( $order, $posted ) {
		...
		WC_Gateway_PayRetailers::log( 'Only handling full refund. Controlling that order total equals amount refunded. Does '.$order->get_total().' equals '.$posted['grand_total'].' ?' );
		if ( $order->get_total() == ( $posted['grand_total'] ) ) {

			// Mark order as refunded
			$order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.', 'payretailers-woocommerce' ), strtolower( $posted['status'] ) ) );

			/*$this->send_ipn_email_notification(
				sprintf( __( 'Payment for order #%s refunded/reversed', 'woocommerce' ), $order->get_order_number() ),
				sprintf( __( 'Order #%s has been marked as refunded - PayRetailers reason code: %s', 'woocommerce' ), $order->get_order_number(), $posted['reason_code'] )
			);*/
		}
	}
	 ...
	 * @param WC_Order $order
	 */
	protected function save_payretailers_meta_data( $order, $posted ) {
		if ( ! empty( $posted['tax'] ) ) {
